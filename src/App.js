...
class App extends Component {
  constructor() {
	...
	componentDidMount() {
	  let cards = [];
	  Object.entries(this.state.modules).forEach(
		  ([module, details]) => {
				cards.push(
				  <Card key={module} as='a' href={details['link']} target='_blank'>
					  <Card.Content textAlign='center'>
					    ...
					    <Image centered src={require(`./template/img/${details['logo']}`)} />
						</Card.Content>
					</Card>
				);
			}
  ...
  render() {
    return (
      <Container text>
			  <br />
			  <Intro title={this.state.site.title} subtitle={this.state.site.subtitle}/>
